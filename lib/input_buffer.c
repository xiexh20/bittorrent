...
/**
 * process the user commandline input
 * fd: the file descriptor
 * userbuf: the buffer to receive input
 * handle_line: the callback function to process input
 * cdata: the parameter to the callback function
 * no return value
 */
                        ...
                        void (*handle_line)(char *, void *), void *cbdata)
{
   ...
   * implement some other form of error handling */
    ...
    {
        fprintf(stderr,
                "process_user_input error:  buffer full;  line too long!\n");
        exit(-1);
    }
