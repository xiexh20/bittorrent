...
/**
 * config: information to config the peer: map, node, chunks, file location. 
 * argc: number of input arguments
 * **argv: user command input values
 */
...
void bt_usage()
{
    fprintf(stderr, "usage:  peer [-h] [-d <debug>] -p <peerfile> -c <chunkfile> "
                    "-m <maxconn>\n"
...
void bt_parse_command_line(bt_config_t *config)
{
    ...
    while ((c = getopt(argc, argv, _bt_optstring)) != -1)
    {
        ...
        case 'd':
            ...
            {
                fprintf(stderr,
                        "%s:  Invalid debug argument.  Use -d list  to see debugging "
                        "options.\n",
                        argv[0]);
                exit(-1);
            }
    ...
    {
        fprintf(stderr,
                "bt_parse error:  No peer information for myself (id %d)!\n",
                config->identity);
        exit(-1);
    }
...
void bt_dump_config(bt_config_t *config)
{
    ...
    for (p = config->peers; p; p = p->next)
        printf("  peer %d: %s:%d\n", p->id, inet_ntoa(p->addr.sin_addr),
